...
Потребуется только указать атрибут для класса компонента данных:
```csharp
using Pixeye.Actors;
using UnityEngine;
...
Поэтому для доступа к компоненту *из групп* используйте:
```csharp
...
```csharp
	public static class Component
	{
		 ...
		 * используйте для этого предыдущий метод.
		 */
		public static void FromGroup<T>(in this ent entity, out T component) where T: class
		  => component = Storage<T>.components[entity.id];
	}
```
Такой способ не создает проблем с переименованием.  

**Создание нового компонента через `entity.Set<T>()` станет дороже**, т.к. будет использоваться реализация `Create<T>` по умолчанию - `Activator.Create<T>`(однако благодаря пулу это будет смягчено). Это медленнее, чем прямой вызов конструктора. Такого поведения в критичных к производительности местах можно избежать, если использовать другую реализацию - `entity.Set<T>(T component)`:
```csharp
var cPlayer = new ComponentPlayer();
entity.Set(cPlayer);
```
**TODO(невыяснено поведение хранилища - хранит ли оно релизнутые компоненты всегда или позволяет заменять их) 
**Dispose останется не реализованным**, поэтому за сброс значений компонентов сущности после `entity.Release` отвечаете вы. (в `entity.Set<T>()` вам может попасться старый компонент от релизнутой сущности).
